...
const updateLeagues = async (leagues) => {
  const leagues_to_update = await Promise.all(
    leagues.map(async (league) => {
      const rosters = await fetchLeagueRosters(league.league_id);

      const roster_user_change = rosters.find((r) =>
        league.rosters.find(
          (r2) =>
            r.roster_id === r2.roster_id &&
            (r.owner_id !== r2.user_id ||
              r2.co_owners
                .map((co) => co.user_id)
                .some((user_id) => r.co_owners.includes(user_id)) ||
              r.co_owners.some((user_id) =>
                r2.co_owners.map((co) => co.user_id).includes(user_id)
              ))
        )
      );
      ...
      if (roster_user_change) {
        const users = await fetchLeagueUsers(league.league_id);

        updated_rosters = updateLeagueRostersUsers(rosters, users);
      }
    })
  );
};

const addLeagues = async (leagues) => {
  ...
  await Draft.bulkCreate(draft_data, { updateOnDuplicate: ["type", "status"] });
